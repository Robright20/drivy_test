...
def array_of_rental_price
	...
	Rentals.all.each do |rental|
		...
		rental_prices << { :id => rental.id, :options => Option.type_by(rental.id), :actions => build_actions_array(car, rental, {:price => rental_price(car, rental), :commission => build_hash_of_commission(car, rental) })}
	end
